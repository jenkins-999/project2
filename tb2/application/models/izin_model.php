...
class izin_model extends CI_Model
{
	...
	public function read($id_karyawan = '')
	{
		...
		$this->db->from('izin');
		$this->db->join('karyawan', 'izin.id_karyawan = karyawan.nomor_pegawai');

		//filter data sesuai id yang dikirim dari controller
		if ($id_karyawan != '') {
			$this->db->where('izin.id_karyawan', $id_karyawan);
		}

		$this->db->order_by('izin.id ASC');
		$query = $this->db->get();
	...
	public function insert($input)
	{
		...
		return $this->db->insert('izin', $input);
	}
	...
	public function update($input, $id)
	{
		...
		//$input = data yang dikirim dari controller
		return $this->db->update('izin', $input);
	}
	...
	public function laporan($id_karyawan = '', $bulan = '')
	{
		...
		$this->db->join('karyawan', 'izin.id_karyawan = karyawan.nomor_pegawai');;
		if ($bulan != '') {
			$this->db->where('DATE_FORMAT(tgl_izin,"%m")', $bulan);
		}
	...
	{
		$this->db->select('izin.*');
		$this->db->select('COUNT(status) AS data_izin');
		$this->db->from('izin');
